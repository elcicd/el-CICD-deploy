...
+
Converting an already working Helm chart into a el-CICD Chart compatible chart may not be feasible and/or is probably not worth the time.  el-CICD Chart can be used as a general YAML templating tool to generate values.yaml files for other Helm charts via Helm https://helm.sh/docs/topics/advanced/#post-rendering[post-renderering].
...
<.> Both `objNames` and `namespaces` can use patterns to further decorate each final matrix value.
.. $<> refers each element in the matrix; e.g. `job-en` or `job-es`.
.. $<#> refers to the index of the element in the matrix; e.g. the pattern `$<>-$<#>` would generate `job-en-1` and `job-en-2`.
.. In the deployment definition above, the __base name__ is decorated with either `-helper` or `-raw` so the pods generated from either the helper template or the raw YAML templates are clearly identified.
.. In templates generated from matrices with patterns, use `$<BASE_OBJ_NAME>` to refer to the undecorated element from the matrix's list that generated `$<OBJ_NAME>`.

<.> The `template` helper attribute contains the free form YAML definition.
.. Matrices, variables, and deployment profiles can be used as usual with free form el-CICD Chart template.
.. The `metadata` section can optionally defined directly in.
... el-CICD Chart will attempt generate a `metadata` automatically if it is missing.
... Any fields defined in `metadata` sections will always take precedence over values defined in helper attributes.
... If `kubeObject: false` is set on the el-CICD Chart template, the metadata section will not be generated if missing.

<.> Defining environment variables in a container is the same whether using helper or raw templates.

NOTE: There is rarely if ever a reason to use a raw template when cl-CICD Chart helper templates exist.  This example is for demonstration purposes only.

[discrete]
==== Tutorial steps
:helm-update-command: helm upgrade --install --rollback-on-failure -n elcicd-chart-demo --create-namespace -f elcicd-demo-5.yaml elcicd-chart-demo elcicd-charts/elcicd-chart
